...
let runTime = false; // It use as a control
...
function animateWatch() {
  ...
  if (allSeconds === 0) {
    ...
    // After the clearInterval, use runTime as a false value for can restart play function.
    runTime = false;
  } else {
...
play.addEventListener('click', () => {
  if (runTime === false) {
    ...
    // While setInterval execute, use runTime as a true value for can not restart play function
    runTime = true;
  }
});
...
pause.addEventListener('click', () => {
  ...
  // Use runTime as a false value for can restart play function
  runTime = false;
});